...
TEST(xml_in, abo_loeschen_anfrage) {
  auto const msg = parse(abo_loeschen_str);
  ASSERT_TRUE(holds_alternative<abo_loeschen_anfrage_msg>(msg));
  auto const abo_loeschen_anfrage_actual = get<abo_loeschen_anfrage_msg>(msg);
  EXPECT_EQ(abo_loeschen_anfrage_actual.sender_, "motis");
  EXPECT_EQ(abo_loeschen_anfrage_actual.t_, t);
  ASSERT_EQ(abo_loeschen_anfrage_actual.abo_ids_.size(), 2);
  EXPECT_EQ(abo_loeschen_anfrage_actual.abo_ids_[0], 42);
  EXPECT_EQ(abo_loeschen_anfrage_actual.abo_ids_[1], 23);
}
